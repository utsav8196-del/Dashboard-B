...
} = require("../utils/emailServiceV2");
const {
    schedulePaymentReminder,
    cancelPaymentReminder,
} = require("../utils/jobScheduler");

// ✅ CREATE BILL (WITH AUTO INVENTORY UPDATE & EMAIL V2)
async function createBill(req, res) {
    try {
        ...
        console.log("📝 CREATE BILL - REQ BODY:", req.body);
        ...
        if (paymentStatus === "paid") {
            ...
            await sendPaymentConfirmedEmail(bill, language || "en");
        } else {
            // Send bill created email (for pending bills)
            await sendBillCreatedEmail(bill, language || "en");
...
// ✅ UPDATE PAYMENT STATUS (WITH INVENTORY UPDATE & EMAIL V2)
async function updatePaymentStatus(req, res) {
    try {
        const { paymentStatus, language } = req.body;
